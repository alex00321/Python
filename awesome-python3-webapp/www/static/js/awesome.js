...
function toSmartDate(timestamp) {
    ...
    if (t > 604800000) {
        ...
        // 1-23 hours ago:
        s = Math.floor(t / 3600000) + 'hours agi';
    }
    else if (t >= 60000) {
        s = Math.floor(t / 60000) + 'mins ago';
    }
    return s;
}
...
function _httpJSON(method, url, data, callback) {
    ...
    $.ajax(opt).done(function (r) {
        ...
        return callback({'error': 'http_bad_response', 'data': '' + jqXHR.status, 'message': 'network issue (HTTP ' + jqXHR.status + ')'});
    });
}
